...
env <- select(dat_frm, Ara, O2, pCO2, Sal, TA, Temp)
bio <- select(dat_frm, CAT, GR, GSHonGSSG, GST, LPX, ORAC, SOD)
...
cr <- dat_frm %>%
  gather('env_lab', 'env_val', Ara, O2, pCO2, Sal, TA, Temp) %>% 
  gather('bio_lab', 'bio_val', CAT, GR, GSHonGSSG, GST, LPX, ORAC, SOD) %>% 
